...
func register(w http.ResponseWriter, r *http.Request) {
	...
	e := r.FormValue("e")
	if e == "" {
		msg := url.QueryEscape("email is required")
		http.Redirect(w, r, "/?msg="+msg, http.StatusSeeOther)
		return
	}
...
func login(w http.ResponseWriter, r *http.Request) {
	...
	e := r.FormValue("e")
	if e == "" {
		msg := url.QueryEscape("credentials cannot be empty")
		http.Redirect(w, r, "/?msg="+msg, http.StatusSeeOther)
		return
	}
	...
	p := r.FormValue("p")
	if p == "" {
		msg := url.QueryEscape("complete credentials are required")
		http.Redirect(w, r, "/?msg="+msg, http.StatusSeeOther)
		return
	}

	if _, ok := db[e]; !ok {
		msg := url.QueryEscape("your credentials didn't match")
		http.Redirect(w, r, "/?msg="+msg, http.StatusSeeOther)
		return
	}
